...
climate::ClimateTraits RamsesClimate::traits() {
  ...
  traits.set_supported_presets(
      {climate::CLIMATE_PRESET_NONE, climate::CLIMATE_PRESET_HOME,
       climate::CLIMATE_PRESET_AWAY, climate::CLIMATE_PRESET_ECO,
...
void RamsesClimate::handle_message(const ramses_esp::RamsesMessage &msg) {
  ...
  if (opcode == 0x30C9) {
    ...
    auto dec =
        ramses_esp::SystemModePayload::decode(msg.payload, msg.n_payload);
    if (dec.has_value()) {
      switch (dec->mode) {
...
void RamsesClimate::control(const climate::ClimateCall &call) {
  ...
  if (call.get_preset().has_value()) {
    ...
    case climate::CLIMATE_PRESET_AWAY:
      sys_mode = ramses_esp::SystemMode::AWAY;
      break;
    case climate::CLIMATE_PRESET_ECO:
      sys_mode = ramses_esp::SystemMode::ECO_BOOST;
      break;
    case climate::CLIMATE_PRESET_HOME:
      sys_mode = ramses_esp::SystemMode::DAY_OFF;
      break;
    case climate::CLIMATE_PRESET_COMFORT:
      sys_mode = ramses_esp::SystemMode::CUSTOM;
      break;
