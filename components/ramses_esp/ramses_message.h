...
class RamsesMessageBuilder {
 public:
  ...
  static RamsesMessageBuilder query() { return RamsesMessageBuilder(RAMSES_MSG_RQ); }
  static RamsesMessageBuilder info() { return RamsesMessageBuilder(RAMSES_MSG_I); }
  static RamsesMessageBuilder write() { return RamsesMessageBuilder(RAMSES_MSG_W); }
  static RamsesMessageBuilder reply() { return RamsesMessageBuilder(RAMSES_MSG_RP); }

  RamsesMessageBuilder &type(RamsesMsgType t) {
    msg_.type = t;
    msg_.fields = (msg_.fields & ~RAMSES_F_MASK) | (static_cast<uint8_t>(t) & RAMSES_F_MASK);
    return *this;
  }

  RamsesMessageBuilder &from(const RamsesAddress &src) {
    return addr0(src);
  }

  RamsesMessageBuilder &to(const RamsesAddress &dst) {
    return addr1(dst);
  }
  ...
  RamsesMessageBuilder &addr(size_t index, const RamsesAddress &address) {
    if (index < RAMSES_MAX_ADDR) {
      ...
      if (index == 0) msg_.fields |= RAMSES_F_ADDR0;
      else if (index == 1) msg_.fields |= RAMSES_F_ADDR1;
      else if (index == 2) msg_.fields |= RAMSES_F_ADDR2;
    }
    return *this;
  }
...
inline RamsesMessageBuilder RamsesMessage::builder(RamsesMsgType type) { return RamsesMessageBuilder(type); }
inline RamsesMessageBuilder RamsesMessage::query() { return RamsesMessageBuilder::query(); }
inline RamsesMessageBuilder RamsesMessage::info() { return RamsesMessageBuilder::info(); }
inline RamsesMessageBuilder RamsesMessage::write() { return RamsesMessageBuilder::write(); }
inline RamsesMessageBuilder RamsesMessage::reply() { return RamsesMessageBuilder::reply(); }
