...
template <size_t N> struct FormatString {
  char value[N];
  constexpr FormatString(const char (&str)[N]) { std::copy_n(str, N, value); }
...
template <TypeCode Code> struct FieldTypeMap;

template <> struct FieldTypeMap<TypeCode::U8> {
  using type = uint8_t;
  static constexpr size_t size = 1;
};
template <> struct FieldTypeMap<TypeCode::I8> {
  using type = int8_t;
  static constexpr size_t size = 1;
};
template <> struct FieldTypeMap<TypeCode::BE_U16> {
  using type = uint16_t;
  static constexpr size_t size = 2;
};
template <> struct FieldTypeMap<TypeCode::LE_U16> {
  using type = uint16_t;
  static constexpr size_t size = 2;
};
template <> struct FieldTypeMap<TypeCode::BE_I16> {
  using type = int16_t;
  static constexpr size_t size = 2;
};
template <> struct FieldTypeMap<TypeCode::LE_I16> {
  using type = int16_t;
  static constexpr size_t size = 2;
};
template <> struct FieldTypeMap<TypeCode::BE_U32> {
  using type = uint32_t;
  static constexpr size_t size = 4;
};
template <> struct FieldTypeMap<TypeCode::LE_U32> {
  using type = uint32_t;
  static constexpr size_t size = 4;
};
template <> struct FieldTypeMap<TypeCode::BE_I32> {
  using type = int32_t;
  static constexpr size_t size = 4;
};
template <> struct FieldTypeMap<TypeCode::LE_I32> {
  using type = int32_t;
  static constexpr size_t size = 4;
};
template <> struct FieldTypeMap<TypeCode::PAD> {
  using type = void;
  static constexpr size_t size = 1;
};

namespace detail {
[[noreturn]] inline void unsupported_format_specifier() {}
} // namespace detail

// 3. Compile-time parser & code generator
template <FormatString Fmt> class Struct {
private:
  static consteval auto parse_schema() {
    ...
    for (size_t i = 0; i < Fmt.size(); ++i) {
      ...
      switch (c) {
      case 'B':
        codes[count++] = TypeCode::U8;
        break;
      case 'b':
        codes[count++] = TypeCode::I8;
        break;
      case 'H':
        codes[count++] =
            (endian == Endianness::BIG) ? TypeCode::BE_U16 : TypeCode::LE_U16;
        break;
      case 'h':
        codes[count++] =
            (endian == Endianness::BIG) ? TypeCode::BE_I16 : TypeCode::LE_I16;
        break;
      case 'I':
        codes[count++] =
            (endian == Endianness::BIG) ? TypeCode::BE_U32 : TypeCode::LE_U32;
        break;
      case 'i':
        codes[count++] =
            (endian == Endianness::BIG) ? TypeCode::BE_I32 : TypeCode::LE_I32;
        break;
      case 'x':
        codes[count++] = TypeCode::PAD;
        break;
      default:
        detail::unsupported_format_specifier();
        break;
      }
    }
  ...
  template <size_t Idx> static constexpr size_t field_size() {
    switch (codes[Idx]) {
    case TypeCode::U8:
    case TypeCode::I8:
    case TypeCode::PAD:
      return 1;
    case TypeCode::BE_U16:
    case TypeCode::BE_I16:
    case TypeCode::LE_U16:
    case TypeCode::LE_I16:
      return 2;
    case TypeCode::BE_U32:
    case TypeCode::BE_I32:
    case TypeCode::LE_U32:
    case TypeCode::LE_I32:
      return 4;
    }
    return 0;
  }
...
public:
  static constexpr size_t size =
      calc_total_size(std::make_index_sequence<field_count>{});

  // Deduction of the unpacked std::tuple return type (filtering out PAD 'x'
  // bytes)
  template <size_t... Is>
  ...
  using TupleType =
      decltype(tuple_type_builder(std::make_index_sequence<field_count>{}));
  ...
  static std::optional<TupleType> unpack_from(std::span<const uint8_t> &buf,
                                              size_t advance_by = size) {
    if (buf.size() < size || buf.size() < advance_by)
      return std::nullopt;
    auto res = unpack(buf);
  ...
  public:
    ...
    public:
      ...
      Iterator(std::span<const uint8_t> buf, size_t stride)
          : buf_(buf), stride_(stride) {
        fetch();
      }
    ...
    Range(std::span<const uint8_t> buf, size_t stride)
        : buf_(buf), stride_(stride) {}
  ...
  static Range unpack_all(const uint8_t *buf, size_t len,
                          size_t stride = size) {
    if (!buf)
      return Range({}, stride);
...
private:
  ...
  static void read_field(const uint8_t *&ptr, TupleT &tup) {
    ...
    if constexpr (code == TypeCode::PAD) {
      ...
      std::get<tuple_index<Idx>()>(tup) =
          (static_cast<uint16_t>(ptr[0]) << 8) | ptr[1];
      ptr += 2;
    } else if constexpr (code == TypeCode::LE_U16) {
      std::get<tuple_index<Idx>()>(tup) =
          (static_cast<uint16_t>(ptr[1]) << 8) | ptr[0];
      ptr += 2;
    } else if constexpr (code == TypeCode::BE_I16) {
      std::get<tuple_index<Idx>()>(tup) =
          static_cast<int16_t>((static_cast<uint16_t>(ptr[0]) << 8) | ptr[1]);
      ptr += 2;
    } else if constexpr (code == TypeCode::LE_I16) {
      std::get<tuple_index<Idx>()>(tup) =
          static_cast<int16_t>((static_cast<uint16_t>(ptr[1]) << 8) | ptr[0]);
      ptr += 2;
      ...
      std::get<tuple_index<Idx>()>(tup) =
          static_cast<int32_t>((static_cast<uint32_t>(ptr[0]) << 24) |
                               (static_cast<uint32_t>(ptr[1]) << 16) |
                               (static_cast<uint32_t>(ptr[2]) << 8) | ptr[3]);
      ptr += 4;
    } else if constexpr (code == TypeCode::LE_I32) {
      std::get<tuple_index<Idx>()>(tup) =
          static_cast<int32_t>((static_cast<uint32_t>(ptr[3]) << 24) |
                               (static_cast<uint32_t>(ptr[2]) << 16) |
                               (static_cast<uint32_t>(ptr[1]) << 8) | ptr[0]);
      ptr += 4;
    }
  ...
  template <size_t TargetIdx> static consteval size_t tuple_index() {
    size_t non_pad_idx = 0;
    for (size_t i = 0; i < TargetIdx; ++i) {
      if (codes[i] != TypeCode::PAD)
        non_pad_idx++;
    }
    return non_pad_idx;
  }
...
namespace binary {
template <FormatString Fmt> using Struct = ::esphome::ramses_esp::Struct<Fmt>;
}
