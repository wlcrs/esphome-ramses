...
**Status:** Current static DHW/OpenTherm scope complete. Future fault-code and
schedule/configuration support remains pending.
...
1. [done] Payload variants required by currently supported heating, DHW, HVAC, and
  OpenTherm devices.
2. [done] Fault and diagnostic state payloads needed for useful static diagnostics.
  Fault-code payloads remain pending until a concrete static YAML use case is
  identified.
...
4. [done] Additional binding and commissioning payloads only where they support the existing
   fan pairing workflow.

Each opcode addition must include:

- [done] A C++ decoder or encoder with a `ramses_rf` reference.
- [done] Valid-length, truncated, sentinel, and variant tests.
- [done] At least one real packet from the local corpus.
- [done] Differential comparison against `ramses_rf`.
- [done] An explicit statement of whether it is telemetry-only, query-capable, or writable.

## Phase 6: ESPHome Configuration and Compatibility Validation

**Status:** [done] Representative static configuration and ESP32-C6 firmware
validation complete. Discovery remains a separate external workflow.

- [done] Compile representative static YAML generated by the external discovery component.
- [done] Add examples covering one heating system, one MVHR system, one DHW system, and one
  OpenTherm system.
- [done] Validate multiple entities sharing the same `ramses_esp` parent.
- [done] Validate multiple zones and multiple sensors for one controller.
- [done] Check that all generated configuration keys remain stable and documented.
- [done] Keep discovery-component tests separate; here, only consume its generated YAML
  contract.
