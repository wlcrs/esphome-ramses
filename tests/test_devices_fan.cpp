...
void test_fan_rx_status() {
  ...
  RamsesMessage boost_msg =
      parse_msg("045  I --- 32:155617 --:------ 32:155617 22F1 003 0006FF");
...
void test_fan_control_tx() {
  ...
  RamsesMessage low_write = FanStatePayload::encode_write(
      hgi_src, dev, FanPresetMode::LOW, HvacScheme::ORCON);
  TEST_ASSERT(low_write.payload[0] == 0x00 && low_write.payload[1] == 0x01,
              "Payload contains Orcon low code 0x01");
...
void test_binding_codec() {
  ...
  RamsesMessage offer_msg =
      parse_msg("073  I --- 37:171871 --:------ 37:171871 1FC9 024 "
  ...
  TEST_ASSERT(offer_dec->bindings[2].oem_code == 0x67,
              "10E0 binding has Orcon OEM code 0x67");

  // Test decode of Accept response:
  // W --- 32:155617 37:171871 --:------ 1FC9 012 0031D9825FE10031DA825FE1
  RamsesMessage accept_msg =
      parse_msg("070  W --- 32:155617 37:171871 --:------ 1FC9 012 "
  ...
  TEST_ASSERT(accept_dec->bindings.size() == 2,
              "Accept has 2 binding tuples (31D9, 31DA)");
  TEST_ASSERT(accept_dec->bindings[0].opcode == 0x31D9,
              "First accept binding is 31D9");
  TEST_ASSERT(accept_dec->bindings[1].opcode == 0x31DA,
              "Second accept binding is 31DA");

  // Test decode of Confirm:
  // I --- 37:171871 32:155617 --:------ 1FC9 001 00
  RamsesMessage confirm_msg =
      parse_msg("072  I --- 37:171871 32:155617 --:------ 1FC9 001 00");
  ...
  RamsesMessage encoded_offer =
      BindingPayload::encode_offer(remote, HvacScheme::ORCON);
  ...
  TEST_ASSERT(encoded_offer.payload[1] == 0x22 &&
                  encoded_offer.payload[2] == 0xF1,
  ...
  RamsesMessage vasco_offer =
      BindingPayload::encode_offer(remote, HvacScheme::VASCO);
  ...
  RamsesMessage encoded_confirm =
      BindingPayload::encode_confirm(remote, fan_addr);
  TEST_ASSERT(encoded_confirm.type == RAMSES_MSG_I,
              "Encoded confirm is I type");
...
void test_pairing_handshake_simulation() {
  ...
  TEST_ASSERT(fan.get_remote_address().is_valid,
              "Remote address valid after setup()");
  TEST_ASSERT(fan.get_remote_address().dev_class == 37,
              "Derived remote uses class 37 (DIS switch)");
  ...
  // Simulate receiving a 1FC9 Accept from the HRU addressed to our derived
  // remote Build packet using the actual derived address
  ramses_esp::RamsesAddress rem = fan.get_remote_address();
  uint8_t rb[3];
  rem.to_bytes(rb);

  // Construct: W --- 32:137527 <remote> --:------ 1FC9 012
  // 0031D9825FE10031DA825FE1
  char accept_frame[200];
  snprintf(
      accept_frame, sizeof(accept_frame),
  ...
  TEST_ASSERT(fan.get_device_address().is_valid,
              "Device address discovered from Accept");
  TEST_ASSERT(fan.get_device_address().dev_class == 32,
              "Discovered device class is 32 (HRU)");
  ...
  RamsesMessage wrong_target =
      parse_msg("070  W --- 32:137527 37:999999 --:------ 1FC9 012 "
                "0031D9825FE10031DA825FE1");
...
void test_chip_id_and_nvs_persistence() {
  ...
  TEST_ASSERT(fan_a.get_remote_address().is_valid,
              "Fan A has valid derived remote address");
  TEST_ASSERT(fan_b.get_remote_address().is_valid,
              "Fan B has valid derived remote address");
  TEST_ASSERT(fan_a.get_remote_address() == fan_b.get_remote_address(),
              "Two fans on same chip derive identical remote address");
  TEST_ASSERT(fan_a.get_remote_address().dev_class == 37,
              "Derived address uses device class 37");
  ...
  fan_yaml.setup();
  TEST_ASSERT(fan_yaml.get_remote_address().id == 99999,
              "YAML remote address overrides chip-ID");

  // Verify address u32 serialisation round-trips
  ramses_esp::RamsesAddress orig =
      ramses_esp::RamsesAddress::from_string("32:137527");
  uint32_t packed = addr_to_u32(orig);
  ramses_esp::RamsesAddress roundtrip = u32_to_addr(packed);
  TEST_ASSERT(roundtrip.is_valid,
              "Packed address is valid after u32 round-trip");
