...
def make_case(case: dict[str, object]) -> dict[str, object] | None:
    ...
    if opcode == "0004" and isinstance(payload, dict) and "name" in payload:
        expected.update(zone_index=int(payload["zone_index"], 16), zone_name=payload["name"])
    ...
    elif opcode == "1260" and isinstance(payload, dict) and payload.get("temperature") is not None:
        expected["temperature"] = payload["temperature"]
    elif opcode == "12C0" and isinstance(payload, dict) and "temperature" in payload:
        expected["temperature"] = payload["temperature"]
    elif opcode == "12F0" and isinstance(payload, dict) and payload.get("dhw_flow_rate") is not None:
        ...
        expected.update(domain_or_zone_index=int(payload["zone_index"], 16), demand_pct=payload["heat_demand"] * 100)
    elif opcode == "2E04" and isinstance(payload, dict) and "system_mode" in payload:
        raw_payload = bytes.fromhex(case["hgi80"].split()[-1])
        expected.update(system_mode_raw=raw_payload[0], system_mode=payload["system_mode"])
    elif opcode == "30C9":
        items = payload if isinstance(payload, list) else [payload]
        if not all(isinstance(item, dict) and item.get("temperature") is not None for item in items):
            return None
        expected["temperatures"] = [{"temperature": item["temperature"]} for item in items]
    else:
        return None

    return {"name": f"corpus_{opcode}_{case['length']}", "hgi80": case["hgi80"], "expected": expected}
...
def main() -> int:
    ...
    assert len(cases) >= 12
    with tempfile.NamedTemporaryFile("w", suffix=".json", encoding="utf-8", delete=False) as fixture:
