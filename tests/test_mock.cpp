...
void test_mock_evohome_controller() {
  ...
  hub.set_packet_callback([&](const RamsesMessage &msg, const std::string &hgi80) {
  ...
  TEST_ASSERT(received_hgi80.size() == 1, "Received RP 0004 response from controller");
  ...
  TEST_ASSERT(name.rfind("Lounge", 0) == 0, "Zone name string matches 'Lounge'");
  ...
  received_hgi80.clear();
  std::string w_2309 = "045  W --- 18:005612 01:145678 --:------ 2309 003 0108CA";
  hub.inject_hgi80(w_2309);

  TEST_ASSERT(received_hgi80.size() == 1, "Controller echoed I 2309 setpoint update");
  TEST_ASSERT(ctl->get_zones()[1].target_setpoint == 22.5f, "Controller updated Zone 1 setpoint to 22.5 C");
  ...
  TEST_ASSERT(received_hgi80.size() == 1, "Controller broadcasted I 1F09 mode change");
  TEST_ASSERT(ctl->get_system_mode() == 1, "Controller system mode updated to 1 (Away)");
}
...
void test_mock_mvhr_ventilator() {
  ...
  auto fan = std::make_shared<MockMvhrVentilator>("32:155617", MOCK_SCHEME_ORCON);
  ...
  received_hgi80.clear();
  std::string w_22f1 = "045  W --- 18:005612 32:155617 --:------ 22F1 003 0004FF";
  hub.inject_hgi80(w_22f1);

  TEST_ASSERT(received_hgi80.size() == 1, "Fan broadcasted I 22F1 speed update");
...
void test_mock_trv_and_opentherm() {
  ...
  TEST_ASSERT(received_hgi80.size() == 2, "TRV broadcasted 2 packets (demand 3150 and battery 1060)");

  received_hgi80.clear();
  otb->broadcast_telemetry(hub);
  TEST_ASSERT(received_hgi80.size() == 1, "OpenTherm bridge broadcasted 1 packet (telemetry 3220)");
}
