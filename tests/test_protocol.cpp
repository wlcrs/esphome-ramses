...
void test_manchester_codec() {
  ...
  for (uint8_t nibble = 0; nibble < 16; nibble++) {
    ...
    TEST_ASSERT(manchester_code_valid(encoded),
                "Encoded byte is valid Manchester");
    uint8_t decoded = manchester_decode(encoded);
    TEST_ASSERT(decoded == nibble,
                "Manchester roundtrip matches original nibble");
  }

  // Test invalid Manchester bytes (e.g. 0x00, 0xFF, 0xAA is valid, 0x01 is
  // invalid)
  ...
  TEST_ASSERT(!manchester_code_valid(0x35),
              "0x35 trailer is invalid Manchester");
}
...
void test_ramses_address() {
  ...
  TEST_ASSERT(addr.to_string() == "01:123456",
              "Address string serialization matches");

  RamsesAddress empty_addr = RamsesAddress::from_string("--:------");
  TEST_ASSERT(!empty_addr.is_valid, "Empty address is invalid");
  TEST_ASSERT(empty_addr.to_string() == "--:------",
              "Empty address formats as --:------");

  // Bytes roundtrip
  uint8_t bytes[3];
  addr.to_bytes(bytes);
  RamsesAddress addr2 = RamsesAddress::from_bytes(bytes);
  TEST_ASSERT(addr2.dev_class == addr.dev_class,
              "Byte roundtrip dev_class matches");
...
void test_hgi80_parsing_and_formatting() {
  ...
  std::string sample =
      "045  I --- 01:123456 --:------ 01:123456 1F09 003 0007D0";
  RamsesMessage msg;
  ...
  TEST_ASSERT(msg.payload[0] == 0x00 && msg.payload[1] == 0x07 &&
                  msg.payload[2] == 0xD0,
              "Payload bytes match 0007D0");
  ...
  std::string rq_sample =
      "--- RQ --- 01:054321 01:123456 --:------ 3150 002 0000";
...
void test_raw_frame_construction() {
  ...
  std::string sample =
      "--- I --- 01:123456 --:------ 01:123456 1F09 003 0007D0";
  RamsesMessage msg;
  msg.from_hgi80(sample);

  std::vector<uint8_t> raw = msg.to_raw_frame();
  TEST_ASSERT(raw.size() > 30,
              "Raw frame contains preamble, sync, payload, and trailer");
  // Check preamble
  TEST_ASSERT(raw[0] == 0x55 && raw[1] == 0x55,
              "Preamble begins with 0x55 0x55");
  // Check sync header (FF 00 33 55 53)
  TEST_ASSERT(raw[20] == 0xFF && raw[21] == 0x00 && raw[22] == 0x33 &&
                  raw[23] == 0x55 && raw[24] == 0x53,
